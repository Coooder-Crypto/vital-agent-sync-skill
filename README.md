...
本 Skill 当前固定安装公开 npm 包 `vitalmcp@0.5.2`，不会使用 `sudo`，也不会执行来源不明的远程脚本。WorkBuddy 沙箱无法激活用户级 launchd 服务时，Skill 会把运行时生成的单条命令交给用户在 macOS Terminal 中执行；MCP 必须在 WorkBuddy 中审批、重载并通过原生工具验证。
...
Skill 版本与其固定的 `vitalmcp` 版本保持一致。当前版本为 `0.5.2`。
