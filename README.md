...
本 Skill 当前固定安装公开 npm 包 `vitalmcp@0.5.3`，不会使用 `sudo`，也不会执行来源不明的远程脚本。WorkBuddy Local Preview 会在当前会话中启动局域网接收进程，不要求普通用户打开 Terminal；本地二维码可以在 MCP 审批前显示，但任何健康数据读取仍必须先在 WorkBuddy 中审批 MCP，并通过原生工具验证。
...
Skill 版本与其固定的 `vitalmcp` 版本保持一致。当前版本为 `0.5.3`。
